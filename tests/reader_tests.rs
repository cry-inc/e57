...
#[test]
fn cartesian_invalid_state_single() {
    // This file does not adhere to the E57 spec.
    // Its uses single precision floats to encode the Cartesian invalid state flag (instead of an integer).
    // Files like this are generated by PDAL (not this one, its an artificial small repro case).
    // See also issue https://github.com/cry-inc/e57/issues/18
    let mut e57 = E57Reader::from_file("testdata/cartesian_invalid_state_single.e57").unwrap();
    let pointclouds = e57.pointclouds();
    assert_eq!(pointclouds.len(), 1);
    let pc = pointclouds.first().unwrap();
    assert_eq!(pc.records, 3);
    ...
    assert_eq!(points[2].cartesian, CartesianCoordinate::Invalid);
}
